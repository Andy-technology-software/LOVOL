//
//  NewFeeCollectionPlanViewController.m
//  LOVOL
//
...
@interface NewFeeCollectionPlanViewController ()<UITableViewDelegate,UITableViewDataSource,FromsFillTableViewCellDelegate>
@property(nonatomic,strong)NSArray* typeArr;
@property(nonatomic,strong)NSMutableArray* dataArr;

@end

@implementation NewFeeCollectionPlanViewController
...
- (void)viewDidLoad {
    ...
    //1是填的   2是选的
    self.typeArr = [[NSArray alloc] initWithObjects:@"1",@"2",@"2",@"2",@"1",@"1",@"1", nil];
    [self initUI];
    [self makeSaveBtn];
    [self makeData];
}
 ...
 */
- (void)makeData {
    NSArray* leftArr = [[NSArray alloc] initWithObjects:@"模拟台账名称",@"费用类型",@"收取日期",@"费用比例",@"费用金额",@"费用税率",@"说明", nil];
    NSArray* isMustArr = [[NSArray alloc] initWithObjects:@"1",@"1",@"1",@"1",@"1",@"1",@"0", nil];
    NSArray* placeArr = [[NSArray alloc] initWithObjects:@"请输入模拟台账名称",@"请选择费用类型",@"请选择收取日期",@"请选择费用比例",@"请输入费用金额",@"请输入费用税率",@"请输入说明", nil];
