//
//  OrderProgressInquiryViewController.m
//  LOVOL
//
...
#import "OrderProgressInquiryViewController.h"

#import "OrderProgressInquiryModel.h"

#import "OrderProgressInquiryTableViewCell.h"

#import "LoanProgressInquiryConditionViewController.h"
@interface OrderProgressInquiryViewController ()<UITableViewDelegate,UITableViewDataSource,LoanProgressInquiryConditionViewControllerDelegate>
@property(nonatomic,strong)NSMutableArray* dataArr;

@end

@implementation OrderProgressInquiryViewController
...
- (void)makeData {
    for (int i = 0; i < 10; i++) {
        OrderProgressInquiryModel* model = [[OrderProgressInquiryModel alloc] init];
        model._czr = @"李磊";
        model._dqjd = @"当前节点";
        model._htbh = @"NO.007";
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    OrderProgressInquiryModel *model = nil;
    if (indexPath.row < self.dataArr.count) {
        model = [self.dataArr objectAtIndex:indexPath.row];
    }
    
    return [OrderProgressInquiryTableViewCell hyb_heightForIndexPath:indexPath config:^(UITableViewCell *sourceCell) {
        OrderProgressInquiryTableViewCell *cell = (OrderProgressInquiryTableViewCell *)sourceCell;
        [cell configCellWithModel:model];
    }];
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    static NSString *cellIdentifier = @"OrderProgressInquiryTableViewCell";
    OrderProgressInquiryTableViewCell* cell0 = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
    if (!cell0) {
        cell0 = [[OrderProgressInquiryTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIdentifier];
    }
    cell0.selectionStyle = UITableViewCellSelectionStyleNone;
    OrderProgressInquiryModel* model = self.dataArr[indexPath.row];
