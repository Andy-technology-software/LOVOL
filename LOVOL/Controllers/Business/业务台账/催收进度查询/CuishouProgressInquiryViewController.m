//
//  CuishouProgressInquiryViewController.m
//  LOVOL
//
...
#import "CuishouProgressInquiryViewController.h"

#import "FormsCommonCellModel.h"

#import "FromsDisplayTableViewCell.h"

#import "CuishouProgressInAddViewController.h"//补录

#import "CuishouProgressInquiryContionViewController.h"//查询
@interface CuishouProgressInquiryViewController ()<UITableViewDelegate,UITableViewDataSource,CuishouProgressInquiryContionViewControllerDelegate>
@property(nonatomic,strong)NSMutableArray* dataArr;

@end

@implementation CuishouProgressInquiryViewController
 ...
 */
- (void)makeData {
    NSArray* leftArr = [[NSArray alloc] initWithObjects:@"实体行业",@"经销商",@"合同编号",@"承租人",@"手机号码",@"租赁物信息",@"首期还款日",@"到期日",@"租金余额",@"月租余额",@"逾期租金",@"逾期罚息",@"业务状态", nil];
    NSArray* contentArr = [[NSArray alloc] initWithObjects:@"农业产业",@"开发区经销商",@"NO.001",@"李磊",@"17666666666",@"农机完善",@"2018-01-01",@"2019-01-01",@"￥10000000.00",@"￥100000.00",@"￥100000.00",@"1%",@"正常", nil];
...
- (void)searBClick {
    DLog(@"搜索");
    CuishouProgressInquiryContionViewController* vc = [[CuishouProgressInquiryContionViewController alloc] init];
    vc.title = @"催收进度查询";
    vc.CuishouProgressInquiryContionViewControllerDelegate = self;
...
- (void)makeSaveBtn {
    UIButton* saveBtn = [MyController createButtonWithFrame:self.view.frame ImageName:nil Target:self Action:@selector(saveBtnClick) Title:@"补录"];
...
- (void)saveBtnClick {
    DLog(@"完成");
    CuishouProgressInAddViewController* vc = [[CuishouProgressInAddViewController alloc] init];
    vc.title = @"催收进度补录";
    [self.navigationController pushViewController:vc animated:YES];
}
