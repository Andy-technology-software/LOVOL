...
//
//  LoanApprovalListTableViewCell.m
//  LOVOL
//
...
@interface LoanApprovalListTableViewCell()
...
@end
@implementation LoanApprovalListTableViewCell
...
- (void)configCellWithModel:(LoanApprovalListModel *)model {
    self.numL.text = [NSString stringWithFormat:@"流程编号:%@",model._lcnum];
    
    self.timeL.text = model._time;
    
    self.lcNameL.text = [NSString stringWithFormat:@"流程名称:%@",model._lcname];
    
    self.senderL.text = [NSString stringWithFormat:@"承租人:%@",model._czrName];
    
    self.kehuL.text = [NSString stringWithFormat:@"合同编号:%@",model._htNum];
    
    self.stateL.text = model._dqjd;
