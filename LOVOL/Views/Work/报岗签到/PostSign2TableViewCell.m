...
- (void)makeUI{
    self.backgroundColor = [MyController colorWithHexString:CDefaultColor];
...
- (void)configCellWithModel:(PostSign2Model *)model{
    ...
    [self.bgView mas_remakeConstraints:^(MASConstraintMaker *make) {
        ...
        make.right.mas_equalTo(-12);
        make.top.mas_equalTo(0);
        make.height.mas_offset((50 + imageWidth) * ceil(((float)(model.imageArr.count + 1) / 4)));
    }];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    if (1 == buttonIndex) {
        NSLog(@"删除");
        [self.PostSign2TableViewCellDelegate sendBackDeleImage:self.delNum];
    }
}
- (void)imageBtnClick:(UIButton*)btn{
    if (self.imgaeNum == btn.tag - 2000) {
        [self.PostSign2TableViewCellDelegate sendBackAddImage];
    }else{
        [self.PostSign2TableViewCellDelegate sendBackSelectImage:btn.tag - 2000];
    }
}
